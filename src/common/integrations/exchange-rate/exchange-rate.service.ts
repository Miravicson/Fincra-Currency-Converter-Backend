...
import { ExchangerProviderException } from '@common/integrations/exchanger-provider-exception';
import { Decimal } from '@prisma/client/runtime/client';

@Injectable()
export class ExchangeRateService implements OnModuleInit, ExchangerService {
  ...
  async convert(options: ExchangerInput): Promise<ExchangerResponse> {
    const exchangeRateOptionalString = await this.getExchangeRate(
      options.fromCurrency,
      options.toCurrency,
    );

    if (!exchangeRateOptionalString) {
      throw new ExchangerProviderException('Could not get exchange rate');
    }

    const exchangeRate = new Decimal(exchangeRateOptionalString);

    return {
      convertedAmount: options.amount.mul(exchangeRate),
      exchangeRate,
  ...
  async refreshAllExchangeRates(base = 'USD') {
    const initialData =
      await this.exchangeHttpSvc.fetchExchangeRateForBaseCode(base);
    ...
    const fetchAndStorePromises = Array.from(allCurrencies).map(
      async (code) => {
        const data =
          await this.exchangeHttpSvc.fetchExchangeRateForBaseCode(code);
  ...
  async getExchangeRate(from: string, to: string): Promise<string | null> {
    if (from === to) return '1';
    ...
    if (!rate) {
      await this.refreshAllExchangeRates();
    }
  ...
  async updateExchangeRate(from: string, to: string): Promise<number> {
    const response =
      await this.exchangeHttpSvc.fetchExchangeRateForBaseCode(from);
